...
class RedisCache:
    ...
    def set(self, key, value, ex=6000)-> None:
        ...
        try:
            logger.info(f"Setting Redis cache for key: {key} with value: {value} and expiration: {ex} seconds")
            self.redis_conn.set(key, value, ex=ex)
        except Exception as e:
            logger.exception(f"Error setting Redis cache for key: {key}. Error: {str(e)}")
    ...
    def get(self, key)-> Optional[str]:
        ...
        try:
            ...
            logger.exception(f"Error getting Redis cache for key: {key}. Error: {str(e)}")
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal Server Error")
...
def get_record_by_field(db: Session, model: Any, field: str, value: Any) -> Optional[Any]:
    ...
    logger = AppLogger().get_logger()
    try: 
        logger.info(f"Querying database for {model.__name__} where {field} = {value}")
        return db.query(model).filter(getattr(model, field) == value).first()
    except Exception as e:  
        logger.exception(f"Error querying database for {model.__name__} where {field} = {value}. Error: {str(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal Server Error")

def cache_lookup(key: str, db: Session, model, field: str, value: str, not_found_msg: str, result_key: str)-> dict:
    """
    ...
    try:
        ...
        record = get_record_by_field(db=db, model=model, field=field, value=value)
        #   If not found in DB, raise 404
        if not record:
            logger.exception(f"Cache miss for key: {key}. No record found in database.")
        ...
        logger.info(f"Record found in database for key: {key}. Caching result and returning value.")
        result_value = getattr(record, result_key)
        # Cache the result in Redis for future lookups
        logger.info(f"Caching result for key: {key} with value: {result_value} in Redis")
        redis.set(key=key, value=result_value)
        logger.info(f"Result cached successfully for key: {key}. Returning value.")
        return {"message": URL_EXISTS, result_key: result_value}
    except HTTPException as e:
        logger.exception(f"HTTPException occurred during cache lookup for key: {key}. Detail: {e.detail}")
        raise e
    except Exception as e:
        logger.exception(f"Unexpected error during cache lookup for key: {key}. Error: {str(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal Server Error")