...
from app.config import (SHORT_URL,
                        LONG_URL,
                        URL_SHORTENED_ALREADY,
                        URL_SHORTENED_SUCCESSFULLY, 
                        URL_DOESNT_EXIST)
from app.models import URLModel
from app.utils import cache_lookup, get_record_by_field,shorten_text, RedisCache
...
def shorten_url(long_url: str, db: Session)-> dict:
    ...
    try:
        ...
        if record:
            ...
            logger.info(f"URL found in database. Caching and returning short URL: {record.short_url}")
        ...
        logger.info(f"Creating new short URL: {short_url} for long URL: {long_url}")
        db.add(new_entry)
        ...
        logger.info(f"Caching new short URL: {short_url} for long URL: {long_url}")
        ...
        logger.exception(f"Error occurred while shortening URL: {long_url}. Error: {str(e)}")
        raise e
...
def get_shortened_url(long_url: str, db: Session)-> dict:
    ...
    try:
        ...
        logger.exception(f"Error occurred while retrieving short URL for long URL: {long_url}. Error: {str(e)}")
        raise e
...
def get_long_url(short_url: str, db: Session)-> dict:
    ...
    try:
        ...
        logger.exception(f"Error occurred while retrieving long URL for short URL: {short_url}. Error: {str(e)}")
        raise e
